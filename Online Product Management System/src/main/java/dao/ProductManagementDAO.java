...
public class ProductManagementDAO {
	public static List<Product> getAllProducts()
	{
		...
		try {
			...
			while(rs.next())
			{
				Product product=new Product(rs.getString("prod_id"),rs.getString("prod_name"),rs.getString("prod_category"),rs.getString("prod_price"));
	...
	public static Product getProductById(String productId)
	{
		...
		try
		{
			...
			while(rs.next())
			{
				product=new Product(rs.getString("PROD_ID"),rs.getString("PROD_NAME"),rs.getString("PROD_CATEGORY"),rs.getString("PROD_PRICE"));
			}
		}
		catch(Exception e)
		{
	...
	public static int deleteProduct(String productId)
	{
		...
		{
			Connection conn=DBUtil.getConnecton();
			PreparedStatement ps=conn.prepareStatement("DELETE FROM product WHERE PROID_ID=?");			
			ps.setString(1, productId);
			status = ps.executeUpdate();
		}
		catch(Exception e)
